# -*- coding: utf-8 -*-
"""DES (FIPS 46-3) 纯 Python 实现（教学版，仅支持 ECB 单块/多块）"""
...
_LEFT_ROT = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]
...
def _generate_subkeys(key_bytes):
    bits = _bytes_to_bits(key_bytes)
    key = _permute(bits, _PC1)
    c = key[:28]
    d = key[28:]
    subkeys = []
    for r in _LEFT_ROT:
...
def _des_round(block_bits, subkey):
    left = block_bits[:32]
    right = block_bits[32:]
    new_right = _xor(left, _f(right, subkey))
...
def _des_block(block, subkeys):
    ...
    for i in range(16):
        bits = _des_round(bits, subkeys[i])
    bits = bits[32:] + bits[:32]
    bits = _permute(bits, _FP)
...
def des_encrypt(data: bytes, key: bytes) -> bytes:
    ...
    for i in range(0, len(data), 8):
        out += _des_block(data[i:i + 8], subkeys)
    return bytes(out)


def des_decrypt(data: bytes, key: bytes) -> bytes:
    subkeys = _generate_subkeys(key)[::-1]
    out = bytearray()
    for i in range(0, len(data), 8):
        out += _des_block(data[i:i + 8], subkeys)
    return bytes(out)
...
def des_encrypt_ecb(plaintext: bytes, key: bytes) -> bytes:
    return des_encrypt(_pad8(plaintext), key)


def des_decrypt_ecb(ciphertext: bytes, key: bytes) -> bytes:
    pt = des_decrypt(ciphertext, key)
    pad = pt[-1]
    return pt[:-pad]
...
if __name__ == "__main__":
    ...
    print("DES encrypt:", ct.hex(), "expected 85E813540F0AB405",
          "->", "PASS" if ct.hex() == "85e813540f0ab405" else "FAIL")
